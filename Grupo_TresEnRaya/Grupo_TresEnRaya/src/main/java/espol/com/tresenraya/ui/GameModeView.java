...
public final class GameModeView extends StackPane {
    ...
    public GameModeView(User user, Consumer<GameMode> onModeSelected) {
        ...
        getChildren().addAll(createBackground(), createTint(), createContent());
    }

    private ImageView createBackground() {
        ImageView view = new ImageView(new Image(getClass().getResourceAsStream(BACKGROUND)));
    ...
    private VBox createContent() {
        ...
        Label subtitle = new Label("Hola, " + user.getName() + ". Selecciona cómo quieres jugar.");
        ...
        for (GameMode mode : GameMode.values()) {
            ...
            cards.add(card, index % 2, index / 2);
            if (index == 2) {
        ...
        Label hint = new Label("Puedes cambiar de modo en cualquier momento antes de iniciar una nueva partida.");
        hint.getStyleClass().add("panel-copy");
        hint.setWrapText(true);
        hint.setMaxWidth(620);
        hint.setAlignment(Pos.CENTER);

        VBox content = new VBox(10, eyebrow, title, subtitle, cards, hint);
    ...
    private Button createModeCard(GameMode mode) {
        ...
        VBox content = new VBox(7, icon, title, description);
        ...
        button.setPrefSize(330, 150);
        button.setOnAction(event -> onModeSelected.accept(mode));
        return button;
    }
}
